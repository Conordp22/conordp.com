...
class thesis_boxes {
	...
	private function create($classes, $parent = false) {
		foreach ((array) $classes as $class) {
			$activated = false;
			$lineage = $parent ? (($this->active[$parent]->_lineage ? $this->active[$parent]->_lineage : '') . ($this->active[$parent]->name ? $this->active[$parent]->name : $this->active[$parent]->title) . " &rarr; ") : false;
			if (!empty($this->instances[$class]) && is_array($this->instances[$class]))
				foreach ($this->instances[$class] as $id => $options)
					if (class_exists($class) && is_subclass_of($class, 'thesis_box') && (!$parent || ($parent && $options['_parent'] == $parent))) {
			...
			if (!$activated && class_exists($class)) {
				...
				($box->name ? $this->add[$class] = $box : $this->assign($box));
			}
		}
	}
